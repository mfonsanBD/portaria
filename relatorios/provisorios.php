...
if ($query->rowCount() > 0) {
	$listaProvisorios = $query->fetchAll();
}
...
<header>
	...
	<h1>Relatórios de <?=$array['c'];?> Atendimentos Provisórios na unidade <?=$_SESSION['nomeLocal']?></h1>
</header>
<footer>
...
<table>
	...
	<?php
		if($array['c'] != 0){
			foreach ($listaProvisorios as $lap) {
	?>
				<tr>
					<td align="center"><?=$lap['cracha']?></td>
					<td align="center"><?=$lap['nome_visitante']?></td>
					<td align="center">
						<?php
							echo date('d/m/y H:i', strtotime($lap['data_entrada']));
						?>			
					</td>
					<td align="center"><?=$lap['cpf_visitante']?></td>
					<td align="center"><?=$lap['telefone_visitante']?></td>
					<td align="center"><?=$lap['alfa']?></td>
				</tr>
	<?php
			}
		}else{
	?>
			<tr>
				<td align="center" colspan="6">Nenhum atendimento provisório realizado</td>
			</tr>
	<?php
		}
	?>
